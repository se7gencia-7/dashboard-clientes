...
import { fetchMetaInsights, getConversions } from '@/lib/meta-api';
...
export async function GET(request: NextRequest) {
  ...
  const level = (searchParams.get('level') as 'campaign' | 'account') ?? 'campaign';

  try {
    const raw = await fetchMetaInsights({ adAccountId, accessToken, datePreset, since, until, level });
